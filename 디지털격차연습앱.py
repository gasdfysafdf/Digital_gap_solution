...
# ─────────────────── 동적 폰트 크기 ───────────────────
g_box_font = "42px" if st.session_state.font_size == "large" else "32px"
card_font  = "34px" if st.session_state.font_size == "large" else "26px"
btn_font   = "36px" if st.session_state.font_size == "large" else "28px"
...
.sdgs-banner h2 {{ color: #FFFFFF !important; margin:0; font-size:34px; font-weight:900; letter-spacing:-0.5px; }}
.sdgs-banner-sub {{ color: #e0e7ff !important; font-size:22px; margin:10px 0 16px; font-weight:700; }}
.sdgs-badge {{
...
.report-box h4 {{ color: #065F46 !important; font-weight:900; margin-top:0; font-size:24px; }}
...
with st.sidebar:
    ...
    <div style="background:linear-gradient(135deg,#0EA5E9,#0284C7);
                ...
                text-align:center;font-size:22px;font-weight:800;margin-bottom:12px;">
    ...
    f_choice = st.radio("👵 글씨 크기", ["크게 보기 (추천)", "보통 보기"],
                        index=0 if st.session_state.font_size == "large" else 1)
    new_font = "large" if f_choice == "크게 보기 (추천)" else "normal"
